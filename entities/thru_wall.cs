...
namespace Celeste.Mod.ErrandOfWednesday
{
    ...
    public class ThruWall : Entity
    {
        ...
        public ThruWall(EntityData data, Vector2 offset, EntityID eid) : base(data.Position+ offset)
        {
            ...
            //can neutrals off of these cost stamina?
        ...
        public override void Update()
        {
            ...
            if(player != null)
            {
                ...
                if(pass_through)
                {
                    bool player_left = player.Left <= Position.X;
                    bool player_right = player.Right >= Position.X;
                    ...
                    {
                        if(active_right && (!inside_left  || player.Speed.X < 0))
                        {
                            active_right = false;
                        }
                        if(active_left && (!inside_right || player.Speed.X > 0))
                        {
                            active_left = false;
                        }
                    }
